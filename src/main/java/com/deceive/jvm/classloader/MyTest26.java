 ...
 * @modified By：cherry
 * @version: 1.0
 * @date ：Created in 2019/10/29 21:28
 */
public class MyTest26 {
    ...
    /**
        ...
        如果一个类由类加载器A加载，那么这个类的依赖类也是由相同的类加载器加载的 (前提是该类没有被加载过)

        ContextClassLoader的作用就是为了破坏Java的类加载器委托机制

        应用场景：
        当高层提供了统一的接口让低层去实现(SPI)，同时又要在高层加载(或实例化) 低层的类时 就必须要通过线程上下文类加载器来帮助高层的ClassLoader
        找到并加载该类


        如果我们没有对线程上下文类加载器做任何设置的话，默认是系统类加载器(AppClassLoader)
        由于是在运行期放置到当前的线程中，不管我们程序位于什么场景下(各个类加载器加载的环境下)，
        在任何需要的时候都可以通过Thread.currentThread().getContextClassLoader();
        来取出线程的类加载器来完成相应的操作
    ...
    public static void main(String[] args) {
       // Thread.currentThread().setContextClassLoader(MyTest26.class.getClassLoader().getParent());
        /*
            服务          定义的一套interface/abstract规范
            服务提供者    接口/抽象类的实现类

            为什么加载器可以找到服务提供者的位置，并加载扫描
            参考ServiceLoader类的官方解释
                通过将提供者配置文件放置在资源目录META-INF / services中来标识服务提供者。文件名是服务类型的标准二进制名称。
                该文件包含一个具体的提供程序类的标准二进制名称列表，每行一个。每个名称周围的空格和制表符以及空白行将被忽略。
                注释字符为“＃”（“ \ u0023”，数字符号）；在每一行中，第一个注释字符之后的所有字符都将被忽略。该文件必须使用UTF-8编码。
         */
        //ServiceLoader是一个简单的服务提供者加载设施 加载·服务具体的实现
...
出于加载的目的，服务由单一类型表示，即单一接口或抽象类。 （可以使用一个具体的类，但是不建议这样做。）给定服务的提供者包含一个或多个具体类，这些具体类使用该提供者特定的数据和代码扩展此服务类型。提供者类通常不是整个提供者本身，而是包含足够信息以决定提供者是否能够满足特定请求的代码以及可以按需创建实际提供者的代码的代理。
提供者类的细节往往是高度特定于服务的；没有单个类或接口可能会统一它们，因此此处未定义此类。此功能强制执行的唯一要求是，提供程序类必须具有零参数构造函数，以便可以在加载期间实例化它们。
...
如果一个特定的具体提供程序类在多个配置文件中被命名，或者在同一配置文件中被多次命名，则重复项将被忽略。命名特定提供程序的配置文件不必与提供程序本身位于同一jar文件或其他分发单元中。该提供程序必须可以从最初查询以查找配置文件的同一类加载程序进行访问；请注意，这不一定是实际从中加载文件的类加载器。
...
示例假设我们有一个服务类型com.example.CodecSet，它旨在表示某种协议的编码器/解码器对的集合。在这种情况下，它是一个具有两个抽象方法的抽象类：public abstract Encoder getEncoder（String encodingName）;公共抽象解码器getDecoder（String encodingName）;
每个方法都返回一个适当的对象，如果提供者不支持给定的编码，则返回null。典型的提供程序支持多种编码。
如果com.example.impl.StandardCodecs是CodecSet服务的实则其jar文件还包含一个名为META现，-INF / services / com.example.CodecSet的文件。
此文件包含单行：com.example.impl.StandardCodecs＃标准编解码器
CodecSet类在初始化时创建并保存一个服务实例：私有静态ServiceLoader <CodecSet> codecSetLoader = ServiceLoader.load（CodecSet.class）;
为了找到给定编码名称的编码器，它定义了一个静态工厂方法，该方法将迭代已知和可用的提供者，仅在找到合适的编码器或用尽提供者时返回。公共静态编码器getEncoder（String encodingName）{for（CodecSet cp：codecSetLoader）{编码器enc = cp.getEncoder（encodingName）; if（enc！= null）返回enc;返回null； }
类似地定义了getDecoder方法。
使用说明如果用于提供程序加载的类加载程序的类路径包含远程网络URL，则在搜索提供程序配置文件的过程中将取消引用这些URL。
此活动是正常的，尽管它可能会导致在Web服务器日志中创建令人费解的条目。但是，如果未正确配置Web服务器，则此活动可能导致提供商加载算法错误地失败。
当请求的资源不存在时，Web服务器应返回HTTP 404（未找到）响应。但是，在某些情况下，有时会将Web服务器错误地配置为返回HTTP 200（OK）响应以及有用的HTML错误页面。当此类尝试将HTML页面解析为提供程序配置文件时，这将引发ServiceConfigurationError。解决此问题的最佳方法是修复配置错误的Web服务器，以返回正确的响应代码（HTTP 404）和HTML错误页面。
以来：
1.6
...
The file's name is the fully-qualified binary name of the service's type. The file contains a list of fully-qualified binary names of concrete provider classes, one per line. Space and tab characters surrounding each name, as well as blank lines, are ignored. The comment character is '#' ('\u0023', NUMBER SIGN);
